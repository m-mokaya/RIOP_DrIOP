...
from models.model import Model, EnsembleModel, ModelA
...
class ReinforcementRunner:
    def __init__(self, envelope: GeneralConfigurationEnvelope, config: ReinforcementLearningConfiguration,
        ...
        self._second_agent = Model.load_from_file(config.second_agent)
    ...
    def run(self):
        ...
        for step in range(self._config.n_steps):
            ...
            if seed % 5 == 0:
                seqs, smiles, agent_likelihood = self._sample_unique_sequences(self._agent, self._second_agent, 1, self._config.batch_size)
                first += 1
            else:
                seqs, smiles, agent_likelihood = self._sample_unique_sequences(self._agent, self._second_agent, 2, self._config.batch_size)
                second += 1
            # switch signs
        ...
        print(f'FIRST MODEL: {first}, SECOND MODEL: {second}')
    ...
    def _sample_unique_sequences(self, agent, second_agent, model, batch_size):
        if model == 1:
            seqs, smiles, agent_likelihood = agent.sample_sequences_and_smiles(batch_size)
        else:
            seqs, smiles, agent_likelihood = second_agent.sample_sequences_and_smiles(batch_size)
            agent_likelihood = second_agent.likelihood(seqs)
    ...
    def _update_reset_countdown(self, reset_countdown, mean_score):
        ...
        if self._config.reset:
            ...
            if reset_countdown == self._config.reset:
                self._agent = EnsembleModel.load_from_file(self._config.agent, self._config.second_agent)
                self._optimizer = torch.optim.Adam(self._agent.network.parameters(), lr=self._config.learning_rate)
                reset_countdown = 0
