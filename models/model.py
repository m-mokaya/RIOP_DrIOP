...
class RNN(tnn.Module):
    ...
    def __init__(self, voc_size, layer_size=512, num_layers=3, cell_type='gru', embedding_layer_size=256, dropout=0.,
        ...
        if self._cell_type == 'gru':
            self._rnn = tnn.GRU(self._embedding_layer_size, self._layer_size, num_layers=self._num_layers, dropout=self._dropout, batch_first=True)
        elif self._cell_type == 'lstm':
            self._rnn = tnn.LSTM(self._embedding_layer_size, self._layer_size, num_layers=self._num_layers, dropout=self._dropout, batch_first=True)
        else:
...
class ModelA(tnn.Module):
    def __init__(self, voc_size, layer_size=512, num_layers=3, cell_type='gru', embedding_layer_size=256, dropout=0.,
                 layer_normalization=False) -> None:
    ...
    def forward(self, input_vector, model,  hidden_state=None):
        batch_size, seq_size = input_vector.size()
        if hidden_state is None:
            size = (self._num_layers, batch_size, self._layer_size)
            if self._cell_type == "gru":
...
class EnsembleRNN(tnn.Module):
    """
    Model that combines two priors for reinforcement learning.
    """
    def __init__(self, modelB, voc_size, layer_size=512, num_layers=3, cell_type='gru', embedding_layer_size=256, dropout=0.,
                 layer_normalization=False, ):
    ...
    def second_forward(self, input_vector, hidden_state=None):
        x1, h1 = self._modelB.forward(input_vector)
        return x1, h1        

    def forward(self, input_vector, model, hidden_state=None):
        if model != 1:
            x1, h1 = self._modelB(input_vector)
            return x1, h1   
        else:
            batch_size, seq_size = input_vector.size()
            if hidden_state is None:
...
class EnsembleModel:
    ...
    def __init__(self, vocabulary: mv.Vocabulary, tokenizer, network_params=None, second_network_params=None, max_sequence_length=256,
                 no_cuda=False):
        """
        Implements an RNN.
        ...
        """
        print('INTIALISING ENSEMBLE MODEL.')
        ...
        if not isinstance(network_params, dict):
            ...
            network_params = {}
            second_network_params = {}


        self.networkB = RNN(len(self.vocabulary), **second_network_params)
        self.network = EnsembleRNN(self.networkB, len(self.vocabulary), **network_params)
    ...
    @classmethod
    def load_from_file(cls, modelA_file_path: str, modelB_file_path:str, sampling_mode=False):
        """
        ...
        if torch.cuda.is_available():
            modelA_save_dict = torch.load(modelA_file_path)
            modelB_save_dict = torch.load(modelB_file_path)
        else:
            modelA_save_dict = torch.load(modelA_file_path, map_location=lambda storage, loc: storage)
            modelB_save_dict = torch.load(modelB_file_path, map_location=lambda storage, loc: storage)

        modelA_network_params = modelA_save_dict.get("network_params", {})
        modelB_network_params = modelB_save_dict.get("network_params", {})

        # modelA = Model(
        #     vocabulary=modelA_save_dict['vocabulary'],
        #     tokenizer=modelA_save_dict.get('tokenizer', mv.SMILESTokenizer()),
        #     network_params=modelA_network_params,
        #     max_sequence_length=modelA_save_dict['max_sequence_length']
        # )
        modelB = Model(
            vocabulary=modelB_save_dict['vocabulary'],
            tokenizer=modelB_save_dict.get('tokenizer', mv.SMILESTokenizer()),
            network_params=modelB_network_params,
            max_sequence_length=modelB_save_dict['max_sequence_length']
        )

        #modelA.network.load_state_dict(modelA_save_dict["network"])
        modelB.network.load_state_dict(modelB_save_dict["network"])

        model = EnsembleModel(
            vocabulary=modelA_save_dict['vocabulary'],
            tokenizer=modelA_save_dict.get('tokenizer', mv.SMILESTokenizer()),
            network_params=modelA_network_params,
            second_network_params=modelB_network_params,
            max_sequence_length=modelA_save_dict['max_sequence_length']
        )
        ...
        b_save_dict = {}
        for k, v in modelB_save_dict["network"].items():
            b_save_dict[f"_modelB.{k}"] = v
        
        model_save_dict.update(modelA_save_dict["network"])
        model_save_dict.update(b_save_dict)

        model.network.load_state_dict(model_save_dict)
        print(f'MODEL SAVE DICT: {model_save_dict}')
    ...
    def likelihood(self, sequences) -> torch.Tensor:
        ...
        """
        logits, _ = self.network(sequences[:, :-1], 1)  # all steps done at once
    ...
    def sample_sequences_and_smiles(self, batch_size=128) -> Tuple[torch.Tensor, List, torch.Tensor]:
        ...
        seqs, likelihoods = self._sample(seed, batch_size=batch_size)
        smiles = [self.tokenizer.untokenize(self.vocabulary.decode(seq)) for seq in seqs.cpu().numpy()]
        return seqs, smiles, likelihoods

    # @torch.no_grad()
    def _sample(self, seed, batch_size=128, ) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # NOTE: The first token never gets adcded in the loop so the sequences are initialized with a start token
        hidden_state = None
        nlls = torch.zeros(batch_size)
        first, second = 0, 0 
        for _ in range(self.max_sequence_length - 1):
            if seed % 2 == 0:
                logits, hidden_state = self.network(input_vector.unsqueeze(1), 2, hidden_state=hidden_state)
                first += 1
            else:
                logits, hidden_state = self.network(input_vector.unsqueeze(1), 1, hidden_state=hidden_state)
                second += 1
